...
async def predict_domain(data: PostInput):
    ...
    # 🔥 IMPORTANT: Limit text length (CLIP max 77 tokens)
    ...
    text_features_post = model.encode_text(
        clip.tokenize([combined_text]).to(device)
    )
    ...
    return {
        ...
        "dominant_domain": domain_prompts[max_index]
    }